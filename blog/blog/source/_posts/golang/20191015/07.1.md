...
**注意事项** 如果我们想让数组元素类型为任意类型的话可以使用空接口作为类型（参考 [第 11 章](file://11.9.md)）。当使用值时我们必须先做一个类型判断（参考 [第 11 章](file://11.3.md)）。
...
只有有效的索引可以被使用，当使用等于或者大于 `len(arr1)` 的索引时：如果编译器可以检测到，会给出索引超限的提示信息；如果检测不到的话编译会通过而运行时会 panic:（参考 [第 13 章](file://13.0.md)）
...
在这里i也是数组的索引。当然这两种 for 结构对于切片（slices）（参考 [第 7 章](file://07.2.md)）来说也同样适用。
...
但这在 Go 中并不常用，通常使用切片（参考 [第 7.2 节](file://07.2.md)）。

## 链接

- [目录](https://blog.zshipu.com/go%E5%85%A5%E9%97%A8%E6%95%99%E7%A8%8B/index.html)
- 上一节：[数组与切片](file://07.0.md)
- 下一节：[切片](file://07.2.md)
