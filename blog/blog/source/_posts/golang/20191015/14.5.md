---
title: go入门教程- 14.5 通道、超时和计时器（Ticker）   
...
`time.Tick()` 函数声明为 `Tick(d Duration) <-chan Time`，当你想返回一个通道而不必关闭它的时候这个函数非常有用：它以 d 为周期给返回的通道发送时间，d是纳秒数。如果需要像下边的代码一样，限制处理频率（函数 `client.Call()` 是一个 RPC 调用，这里暂不赘述（参见第 [15.9](file://15.9.md) 节）：
...
- 上一节：[使用select切换协程](file://14.4.md)
- 下一节：[协程和恢复（recover）](file://14.6.md)
