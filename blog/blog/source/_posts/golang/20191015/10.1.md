...
结构体的字段可以是任何类型，甚至是结构体本身（参考第 [10.5](file://10.5.md) 节），也可以是函数或者接口（参考第 11 章）。可以声明结构体类型的一个变量，然后像下面这样给它的字段赋值：
...
结构体类型和字段的命名遵循可见性规则（第 [4.2](file://04.2.md) 节），一个导出的结构体类型中有些字段是导出的，另一些不是，这是可能的。
...
- 上一节：[结构（struct）与方法（method）](file://10.0.md)
- 下一节：[使用工厂方法创建结构体](file://10.2.md)
