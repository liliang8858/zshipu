---
title: go入门教程- 10.2 使用工厂方法创建结构体实例   
...
通过应用可见性规则参考[4.2.1节](file://04.2.md)、[9.5 节](file://09.5.md)就可以禁止使用 new 函数，强制用户使用工厂方法，从而使类型变成私有的，就像在面向对象语言中那样。
...
new 和 make 这两个内置函数已经在第 [7.2.4](file://07.2.md) 节通过切片的例子说明过一次。
...
- 上一节：[结构体定义](file://10.1.md)
- 下一节：[使用自定义包中的结构体](file://10.3.md)
