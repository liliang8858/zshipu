...
如果写入的东西很简单，我们可以使用 `fmt.Fprintf(outputFile, "Some test data.\n")` 直接将内容写入文件。`fmt` 包里的 F 开头的 Print 函数可以直接写入任何 `io.Writer`，包括文件（请参考[章节12.8](file://12.8.md))。
...
（这是一个独立的练习，但是同时也是为[章节15.4](file://15.4.md)做准备）
...
- 上一节：[读取用户的输入](file://12.1.md)
- 下一节：[文件拷贝](file://12.3.md)
