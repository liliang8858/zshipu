---
title: go入门教程- 13.3 从 panic 中恢复（Recover）   
...
`recover` 只能在 defer 修饰的函数（参见 [6.4 节](file://06.4.md)）中使用：用于取得 panic 调用中传递过来的错误值，如果是正常执行，调用 `recover` 会返回 nil，且没有其它效果。
...
log 包实现了简单的日志功能：默认的 log 对象向标准错误输出中写入并打印每条日志信息的日期和时间。除了 `Println` 和 `Printf` 函数，其它的致命性函数都会在写完日志信息后调用 os.Exit(1)，那些退出函数也是如此。而 Panic 效果的函数会在写完日志信息后调用 panic；可以在程序必须中止或发生了临界错误时使用它们，就像当 web 服务器不能启动时那样（参见 [15.4 节](file://15.4.md)中的例子）。
...
- 上一节：[错运行时异常和 panic](file://13.2.md)
- 下一节：[自定义包中的错误处理和 panicking](file://13.4.md)
